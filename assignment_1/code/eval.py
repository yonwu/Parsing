...
class ParseEvaluator:
    ...
    def compute_fscore(self, key_trees, predicted_trees):
        for trees in zip(key_trees, predicted_trees):
            ...
            if len(f1) != len(f2):
                raise ParseError(
                    "Sentence length does not match. Gold sentence length %d, test sentence length %d. Sentence '%s'" % (
                    len(f1), len(f2), " ".join(f1)))

            for gold, test in zip(f1, f2):
                if test != "_RARE_" and gold != test:
                    raise ParseError("Tree words do not match. Gold sentence '%s', test sentence '%s'." % (
                    " ".join(f1), " ".join(f2)))
                    exit()
